...
This vignette provides a **Shiny application** that demonstrates **Async Parallel Execution**. While this version runs in the browser, its asynchronous features (using 'crew' and 'nanonext') are currently **not supported in WebR/Shinylive**, making it unsuitable for interactive parallel execution directly in the browser. For full functionality, please run this application locally in an R environment.

## 🚀 Run this Dashboard Locally (WebR Limitations)

This version of the dashboard runs in your browser using WebAssembly (WebR). However, due to WebR limitations with parallel processing libraries, asynchronous functionality is NOT supported in the browser. Please run this app locally for full async features.
...
cat("\n=== webR DIAGNOSTIC START (dashboard_async.qmd) ===\n")
...
tryCatch({
  ...
  cat("\n✅ webr::install() completed\n")
}, error = function(e) {
  cat("\n❌ ERROR during webr::install():", e$message, "\n\n")
})
...
if ("randomwalk" %in% rownames(installed_pkgs)) {
  cat("✅ randomwalk found:", installed_pkgs["randomwalk", "Version"], "\n")
} else {
  cat("❌ randomwalk NOT found\n")
}
