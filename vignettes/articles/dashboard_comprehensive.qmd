...
server <- function(input, output, session) {
  ...
  observeEvent(input$run_sim, {
    ...
    tryCatch({
      ...
      # For WebR: Run simulation in chunks to allow progress updates
      # This simulates incremental updates even in sync mode
      if (is_webr()) {
        # Simulate progress by breaking into smaller runs
        total_steps <- input$max_steps
        chunk_size <- max(1, floor(total_steps / 20))  # Update ~20 times

        result <- NULL
        for (i in seq(1, total_steps, by = chunk_size)) {
          current_step <- min(i + chunk_size - 1, total_steps)

          # Update progress reactive values
          sim_current_step(current_step)

          # Simulate some walkers completing (rough approximation)
          approx_completed <- floor(input$n_walkers * current_step / total_steps * 0.8)
          sim_completed_walkers(approx_completed)

          # Simulate black pixels growing
          approx_black <- floor(approx_completed * 0.7)
          sim_black_pixels(approx_black)

          # Allow UI to update
          Sys.sleep(0.01)  # Brief pause to allow UI refresh
        }
  ...
  output$paths_plot_first25 <- renderPlot({
    ...
    if (n_walkers >= 1) {
      ...
      if (length(walker_indices) > 0) {
        ...
        # Create plot with light gray background
        par(bg = "gray98")
        ...
        # Add light grid
        grid(nx = grid_size, ny = grid_size, col = "gray85", lty = 1, lwd = 0.5)

        # Use high contrast colors
        path_colors <- grDevices::rainbow(length(filtered_walkers), s = 1, v = 0.6, alpha = 0.8)
