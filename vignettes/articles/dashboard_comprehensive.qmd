...
server <- function(input, output, session) {
  ...
  observeEvent(input$run_sim, {
    ...
    if (input$n_walkers > max_allowed) {
      sim_error_msg(paste0("Too many walkers: ", input$n_walkers,
                           " exceeds max ", max_allowed))
      sim_state("error"); return()
    }
  ...
  output$stats_quintile_boxplot <- renderPlot({
    ...
    if (is.null(df) || nrow(df) < 5) {
      plot.new()
      text(0.5, 0.5, "Need at least 5 walkers for quintile analysis", cex = 1.2, col = "gray")
      return()
    }
      ...
      print(
        ggplot(df, aes(x = quintile, y = steps)) +
          ...
          labs(
            ...
            subtitle = "Q1 = earliest terminators, Q5 = latest terminators",
  ...
  output$stats_quintile_summary <- renderPrint({
    ...
    cat("=== QUINTILE SUMMARY STATISTICS ===\n\n")
    ...
    cat("\n=== TREND ===\n")
    if (any(is.nan(means)) || means[1] == 0) {
      cat("Some quintiles are empty — trend unavailable\n")
    } else {
...
cat(sprintf(
  "**randomwalk** %s | **Git** %s | **R** %s | **Built** %s",
