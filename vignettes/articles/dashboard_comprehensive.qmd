    ...
    column(12,
      tabsetPanel(
        ...
        tabPanel("Notes",
                 h4("Tips"),
                 tags$ul(
                   tags$li(tags$strong("Larger grid"), " (200-400): More exploration space"),
                   tags$li(tags$strong("Fewer walkers"), " (100-500): Faster simulation"),
                   tags$li(tags$strong("8-hood"), ": Diagonal movement for different patterns"),
...
server <- function(input, output, session) {
  ...
  observeEvent(input$run_sim, {
    ...
    # Initialize grid and walkers for batched execution
    tryCatch({
      grid <- initialize_grid(input$grid_size)
      positions <- generate_walker_positions(input$n_walkers, grid)
      walkers <- lapply(seq_along(positions), function(i) {
        # Only store paths for first 20 walkers (memory efficiency)
        create_walker(i, positions[[i]], input$grid_size, store_path = (i <= 20))
      })

      batch_grid(grid)
      batch_walkers(walkers)
      batch_active(seq_len(input$n_walkers))
      batch_step_count(0L)
  ...
  # === BATCHED SIMULATION LOOP (non-blocking via JS round-trip) ===
  #
  # Architecture: R processes a batch → sends 'batch_done' to JS →
  # JS waits 50ms (browser renders, handles clicks) → JS sends
  # 'next_batch' back to R → R processes next batch.
  #
  # Why not invalidateLater(): In WebR/Shinylive, invalidateLater()
  # re-enters the observe() without yielding the Web Worker thread.
  # The JS main thread never gets a chance to process UI events.
  # The JS round-trip forces a real yield to the browser event loop.
  #
  # Kick off first batch when simulation starts.
  # Delay 200ms to ensure JS 'batch_done' handler is registered.
  observe({
    req(sim_state() == "running")
    req(!is.null(batch_grid()))
    invalidateLater(200)  # one-shot: only fires while state == "running"
    isolate({
  ...
  # Each batch triggered by JS round-trip
  observeEvent(input$next_batch, {
    ...
    active <- batch_active()
    grid <- batch_grid()
    walkers <- batch_walkers()
    step_count <- batch_step_count()
    term_counter <- batch_term_counter()
    grid_size <- input$grid_size
    nb <- input$neighborhood    # pre-resolve once (no match.arg per walker)
    bd <- input$boundary
    max_steps <- input$max_steps
    is_4hood <- (nb == "4-hood")
    is_wrap <- (bd == "wrap")

    # Adaptive: target ~100k walker-step ops per batch.
    # 200 walkers → 500 steps; 3000 walkers → 33 steps.
    STEPS_PER_BATCH <- max(5L, as.integer(100000 / max(1L, length(active))))
    ...
    tryCatch({
      for (b in seq_len(STEPS_PER_BATCH)) {
        ...
        for (i in active) {
          ...
          # === INLINE FAST STEP (no match.arg, no logging, no path) ===
          r <- w$pos[1]
          cc <- w$pos[2]
          if (is_4hood) {
            d <- sample.int(4L, 1L)
            np <- switch(d, c(r-1L,cc), c(r+1L,cc), c(r,cc+1L), c(r,cc-1L))
          } else {
            d <- sample.int(8L, 1L)
            np <- switch(d, c(r-1L,cc), c(r+1L,cc), c(r,cc+1L), c(r,cc-1L),
                            c(r-1L,cc+1L), c(r-1L,cc-1L), c(r+1L,cc+1L), c(r+1L,cc-1L))
          }
          if (is_wrap) {
            np <- c(((np[1]-1L) %% grid_size)+1L, ((np[2]-1L) %% grid_size)+1L)
          } else if (np[1]<1L || np[1]>grid_size || np[2]<1L || np[2]>grid_size) {
            w$active <- FALSE; w$termination_reason <- "hit_boundary"
            w$steps <- w$steps + 1L; w$pos <- np
            walkers[[i]] <- w
            if (is.null(w$termination_order)) {
              term_counter <- term_counter + 1L; w$termination_order <- term_counter
              walkers[[i]] <- w; newly_inactive <- c(newly_inactive, i)
            }
            next
          }
          w$pos <- np; w$steps <- w$steps + 1L

          # === INLINE FAST TERMINATION CHECK ===
          if (grid[np[1], np[2]] == 1L) {
            w$active <- FALSE; w$termination_reason <- "touched_black"
          } else if (w$steps >= max_steps) {
            w$active <- FALSE; w$termination_reason <- "max_steps"
          } else {
            # Vectorized black neighbor check
            nr <- np[1]; nc <- np[2]
            if (is_4hood) {
              rows <- c(nr-1L, nr+1L, nr, nr)
              cols <- c(nc, nc, nc-1L, nc+1L)
            } else {
              rows <- c(nr-1L,nr+1L,nr,nr,nr-1L,nr-1L,nr+1L,nr+1L)
              cols <- c(nc,nc,nc-1L,nc+1L,nc-1L,nc+1L,nc-1L,nc+1L)
            }
            valid <- rows>=1L & rows<=grid_size & cols>=1L & cols<=grid_size
            if (any(valid) && any(grid[cbind(rows[valid],cols[valid])] == 1L)) {
              w$active <- FALSE; w$termination_reason <- "black_neighbor"
            }
          }

          if (!w$active && w$termination_reason == "black_neighbor") {
            grid[np[1], np[2]] <- 1L
          }
          if (!w$active && is.null(w$termination_order)) {
            term_counter <- term_counter + 1L; w$termination_order <- term_counter
            newly_inactive <- c(newly_inactive, i)
          }
          walkers[[i]] <- w
        }
        if (length(newly_inactive) > 0L) {
          active <- setdiff(active, newly_inactive)
        }
      }
      ...
      sim_completed_walkers(input$n_walkers - length(active))
      ...
      if (length(active) == 0L) {
        ...
        result <- list(
          ...
          statistics = list(
            ...
            black_percentage = round(100 * sum(grid == 1L) / (grid_size^2), 2),
            grid_size = grid_size,
            total_walkers = input$n_walkers,
            completed_walkers = input$n_walkers,
