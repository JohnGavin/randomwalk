...
ui <- fluidPage(
    ...
    column(12,
      tabsetPanel(
        ...
        tabPanel("Walker Paths",
                 ...
                 helpText("Select walkers by termination order. Paths stored for first/last 200 walkers by ID."),
  ...
  fluidRow(
    column(12,
      ...
      # Real-time progress display (following Shinylive timer example)
      tags$div(
      ...
      # MANDATORY: Dashboard creation timestamp
      tags$div(
        style = "margin-top: 20px; padding-top: 10px; border-top: 1px solid #ddd; font-size: 11px; color: #666; text-align: right;",
...
server <- function(input, output, session) {
  ...
  output$paths_plot_first25 <- renderPlot({
    ...
    if (n_walkers >= 1) {
      ...
      if (length(walker_indices) > 0) {
        ...
        # Add caption text explaining symbols
        mtext("Symbols: Circle = Start Position | Square = End Position | Dots = Walker Path",
  ...
  # Hypothesis test: Exponential decay vs constant survival
  output$hypothesis_plot <- renderPlot({
    ...
    # Fit models
    # H0: Constant survival (intercept only)
    null_model <- lm(steps ~ 1, data = survdat)

    # H1: Linear decay
    linear_model <- lm(steps ~ order, data = survdat)

    # H1b: Log-linear decay (exponential relationship)
    survdat$log_steps <- log(survdat$steps + 1)  # +1 to handle steps=0
    decay_model <- lm(log_steps ~ order, data = survdat)

    # Calculate predictions
    survdat$null_pred <- fitted(null_model)
    survdat$decay_pred <- exp(fitted(decay_model))

    # Model comparison using F-test
    anova_result <- tryCatch({
      anova(null_model, linear_model)
    ...
    # Coefficient from decay model (negative = decay)
    decay_coef <- coef(decay_model)[2]
    half_life <- ifelse(decay_coef < 0, -log(2) / decay_coef, NA)

    # Plot
    ggplot(survdat, aes(x = order, y = steps)) +
      geom_point(alpha = 0.4, size = 1.5, color = "gray40") +
      geom_hline(yintercept = mean(survdat$steps), color = "blue",
                 linetype = "dashed", linewidth = 1) +
      geom_line(aes(y = decay_pred), color = "red", linewidth = 1.2) +
      annotate("text", x = max(survdat$order) * 0.65, y = max(survdat$steps) * 0.9,
               label = sprintf("H0 (constant): mean = %.0f steps\nH1 (decay): half-life = %.0f walkers\np-value = %.4f",
                               mean(survdat$steps),
                               ifelse(is.na(half_life), Inf, half_life),
                               p_value),
               hjust = 0, size = 3.5,
               color = ifelse(p_value < 0.05, "red", "blue")) +
      labs(
        title = "Hypothesis Test: Does Survival Time Decay?",
        subtitle = ifelse(p_value < 0.05,
                          "REJECT H0: Later walkers survive fewer steps (exponential decay)",
                          "FAIL TO REJECT H0: Survival time is approximately constant"),
        x = "Walker Termination Order (chronological)",
        y = "Steps Survived",
        caption = "Blue dashed: H0 (constant mean) | Red curve: H1 (exponential decay)"
      ) +
      theme_minimal(base_size = 12) +
      theme(
        plot.title = element_text(hjust = 0.5, face = "bold"),
  ...
  output$hypothesis_stats <- renderPrint({
    ...
    cat("=== HYPOTHESIS TEST: SURVIVAL DECAY ===\n\n")
    cat("H0: Survival time is constant (no effect of termination order)\n")
    cat("H1: Survival time decays geometrically as simulation progresses\n\n")

    cat("--- Linear Model (steps ~ order) ---\n")
    cat(sprintf("Intercept: %.1f steps\n", coef(linear_model)[1]))
    cat(sprintf("Slope: %.3f steps per walker\n", coef(linear_model)[2]))
    cat(sprintf("R-squared: %.4f\n", summary(linear_model)$r.squared))

    cat("\n--- Log-Linear Model (log(steps) ~ order) ---\n")
    decay_coef <- coef(decay_model)[2]
    cat(sprintf("Decay coefficient: %.6f\n", decay_coef))
    if (decay_coef < 0) {
      half_life <- -log(2) / decay_coef
      cat(sprintf("Half-life: %.1f walkers (time for median survival to halve)\n", half_life))
    } else {
      cat("No decay detected (positive or zero coefficient)\n")
    }
    ...
    if (!is.null(anova_result)) {
      f_stat <- anova_result$F[2]
      p_value <- anova_result$`Pr(>F)`[2]
      cat(sprintf("F-statistic: %.2f\n", ifelse(is.na(f_stat), 0, f_stat)))
      cat(sprintf("p-value: %.4f\n", ifelse(is.na(p_value), 1, p_value)))

      if (!is.na(p_value) && p_value < 0.05) {
        cat("\n*** CONCLUSION: REJECT H0 ***\n")
        cat("Evidence supports exponential/geometric decay of survival time.\n")
        cat("Later walkers encounter black pixels faster.\n")
      } else {
        cat("\n*** CONCLUSION: FAIL TO REJECT H0 ***\n")
        cat("No significant evidence for decay in survival time.\n")
      }
    } else {
      cat("Unable to compute F-test.\n")
    }
  })
