# Wait for workflows to complete and merge PR #9 (telemetry vignette)
library(gh)
...
while (TRUE) {
  ...
  runs <- gh::gh(sprintf("GET /repos/%s/actions/runs", repo),
                 event = "pull_request",
                 branch = "feature/telemetry-vignette-8",
                 per_page = 10)
...
merge_result <- gh::gh(
  ...
  merge_method = "merge",
  commit_title = "Add telemetry vignette with targets pipeline and pkgdown configuration (#8)",
  commit_message = "Merging PR #9 that closes issue #8"
)
