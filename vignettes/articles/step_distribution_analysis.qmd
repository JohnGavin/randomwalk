...
description: "Statistical analysis of random walk step distributions across grid sizes and walker counts using targets pipeline"
format:
  html:
    code-fold: true
...
vignette: >
  %\VignetteIndexEntry{Step Distribution Analysis}
...
::: {.callout-note collapse="true"}
## Build Information (click to expand)
...
get_build_info <- function() {
  sha_full <- tryCatch(git_log(max = 1)$commit, error = function(e) NA)
  sha_short <- if (!is.na(sha_full)) substr(sha_full, 1, 7) else "N/A"
  sha_link <- if (!is.na(sha_full)) {
    sprintf("[%s](https://github.com/JohnGavin/randomwalk/commit/%s)", sha_short, sha_full)
  } else {
    "N/A"
  }
...
get_build_info() %>%
  knitr::kable(caption = "Build Information", align = c("l", "l"), row.names = FALSE)
```
:::
