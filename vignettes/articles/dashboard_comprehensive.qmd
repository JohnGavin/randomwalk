...
server <- function(input, output, session) {
  ...
  # Runtime estimate (WebR performance varies: ~50-500 steps/sec depending on grid size)
  output$runtime_estimate <- renderUI({
    # Actual runtime is proportional to max_steps, not n_walkers * max_steps
    # All walkers move simultaneously each step
    # Performance degrades with larger grids due to neighbor checking
    grid_area <- input$grid_size^2

    # Empirical performance model based on observed runtimes:
    # Small grids (≤100x100): ~200-500 steps/sec
    # Medium grids (100-200): ~100-200 steps/sec
    # Large grids (>200): ~50-100 steps/sec
    if (grid_area <= 10000) {
      steps_per_sec <- 300
    ...
    # Total simulation steps (not walker*steps)
    est_seconds <- input$max_steps / steps_per_sec
  ...
  output$elapsed_time <- renderText({
    ...
    if (sim_state() == "running" && !is.null(sim_start_time())) {
      ...
      sprintf("Elapsed: %02d:%02d", mins, secs)
