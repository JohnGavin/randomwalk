...
ui <- fluidPage(
  ...
  sidebarLayout(
    ...
    sidebarPanel(
      ...
      # Note about WebR limitations
      tags$small(style = "color: #666; display: block; margin-bottom: 10px;",
                 "Note: Timer updates after completion (WebR limitation)"),
...
server <- function(input, output, session) {
  ...
  # Render button with integrated status/timer
  output$run_button_ui <- renderUI({
    # Note: In WebR, the UI won't update during simulation due to single-threaded execution
    # The button will show "Running..." during execution and update to completion time after

    if (sim_state() == "running") {
      # Show disabled button with running status
      # Note: This won't actually display during execution in WebR
      actionButton("run_sim_disabled", "Running... (please wait)",
