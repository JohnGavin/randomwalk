---
title: "Targets Pipeline: Reproducible Workflows with Nested Parallelism"
description: "Using targets and crew for scalable, reproducible random walk simulations"
...
vignette: >
  %\VignetteIndexEntry{Targets Pipeline: Reproducible Workflows with Nested Parallelism}
...
- [randomwalk wiki: How Async Dashboard Uses Crew and Targets](https://github.com/JohnGavin/randomwalk/wiki/How-Async-Dashboard-Uses-Crew-and-Targets)
...
::: {.callout-note collapse="true"}
## Git Information (click to expand)

```{r git-info, eval=TRUE, echo=FALSE, results='asis'}
if (requireNamespace("gert", quietly = TRUE)) {
  commits <- gert::git_log(max = 1)
  sha_short <- substr(commits$commit, 1, 7)
  sha_full <- commits$commit
  # Create clickable link to GitHub commit
  sha_link <- sprintf("[%s](https://github.com/JohnGavin/randomwalk/commit/%s)",
                       sha_short, sha_full)
  knitr::kable(
    data.frame(
      SHA = sha_link,
      Author = commits$author,
      Date = format(commits$time, "%Y-%m-%d %H:%M"),
      Message = substr(commits$message, 1, 50)
    ),
    caption = "Latest Git Commit",
    row.names = FALSE
  )
} else {
  cat("gert package not available")
}
```
:::
