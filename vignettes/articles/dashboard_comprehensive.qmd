      ...
      if (length(walker_indices) > 0) {
        ...
        for (i in seq_along(filtered_walkers)) {
          ...
          if (!is.null(walker$path) && length(walker$path) > 0) {
            ...
            # Draw path as dots for WebR compatibility
            # Lines and segments don't render reliably in WebR/Shinylive
            if (nrow(path_matrix) > 1) {
              # Draw all path points as small dots
              points(path_matrix[, 2], path_matrix[, 1],
                     pch = 16, col = path_colors[i], cex = 0.8)
            }
