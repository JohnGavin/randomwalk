...
server <- function(input, output, session) {
  ...
  observeEvent(input$run_sim, {
    ...
    tryCatch({
      ...
      # For WebR: Update progress during simulation
      # Following pattern from https://github.com/rstudio/shiny-examples/blob/main/142-reactive-timer/app.R
      # The timer will continue updating even during synchronous computation
      if (is_webr()) {
        # Estimate progress based on elapsed time and expected runtime
        # This provides real-time feedback even though simulation runs synchronously
        est_steps_per_sec <- 200
        est_runtime <- input$max_steps / est_steps_per_sec
  ...
  output$fractal_plot <- renderPlot({
    ...
    # The plot_grid_enhanced function now uses gray85 background by default
    p
  })
  ...
  output$paths_plot_first25 <- renderPlot({
    ...
    if (n_walkers >= 1) {
      ...
      if (length(walker_indices) > 0) {
        ...
        # Create plot with light gray background for contrast
        par(bg = "gray90")
        ...
        for (i in seq_along(filtered_walkers)) {
          ...
          if (!is.null(walker$path) && length(walker$path) > 0) {
            ...
            # Draw path as connected segments - more reliable in WebR
            # Using segments instead of lines() for better WebR compatibility
            if (nrow(path_matrix) > 1) {
              for (j in 1:(nrow(path_matrix) - 1)) {
                segments(x0 = path_matrix[j, 2], y0 = path_matrix[j, 1],
                        x1 = path_matrix[j+1, 2], y1 = path_matrix[j+1, 1],
                        col = path_colors[i], lwd = 3)
              }
            }
        ...
        mtext("Symbols: Circle = Start Position | Square = End Position | Lines = Walker Path",
