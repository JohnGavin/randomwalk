...
ui <- fluidPage(
  ...
  sidebarLayout(
    ...
    sidebarPanel(
      ...
      # Timer updates every 500ms using invalidateLater pattern
      tags$small(style = "color: #666; display: block; margin-bottom: 10px;",
                 "Timer updates every 0.5 seconds during simulation"),
...
server <- function(input, output, session) {
  ...
  # Render button with integrated timer using invalidateLater pattern
  output$run_button_ui <- renderUI({
    # Use invalidateLater to refresh button text during simulation
    if (sim_state() == "running") {
      invalidateLater(500)  # Refresh every 500ms while running
    }

    if (sim_state() == "running" && !is.null(sim_start_time())) {
      # Show live updating timer in button
      elapsed_secs <- as.numeric(difftime(Sys.time(), sim_start_time(), units = "secs"))
      mins <- floor(elapsed_secs / 60)
      secs <- floor(elapsed_secs %% 60)
      actionButton("run_sim_disabled", sprintf("Running... %02d:%02d", mins, secs),
                   class = "btn-warning btn-lg",
                   style = "width: 100%;",
                   disabled = TRUE)
    } else if (sim_state() == "complete" && !is.null(sim_end_time()) && !is.null(sim_start_time())) {
      # Show completion time on button
      total_secs <- as.numeric(difftime(sim_end_time(), sim_start_time(), units = "secs"))
      mins <- floor(total_secs / 60)
      secs <- floor(total_secs %% 60)
      actionButton("run_sim", sprintf("Run Simulation (Last: %02d:%02d)", mins, secs),
  ...
  # Observe block to continuously update UI during simulation
  # This follows the crew/shiny pattern for responsive updates
  observe({
    if (sim_state() == "running") {
      # Keep refreshing UI while simulation runs
      invalidateLater(500)
    }
  })
  ...
  output$fractal_plot <- renderPlot({
    ...
    # Enhanced plot with pixel statistics in title
    stats <- result$statistics
    black_count <- stats$black_pixels
    total_pixels <- input$grid_size^2
    black_percent <- round(100 * black_count / total_pixels)

    title <- sprintf("DLA Fractal: %d black pixels (%d%% of %dx%d grid)",
                    black_count, black_percent, input$grid_size, input$grid_size)
    plot_grid(result, main = title, show_legend = FALSE)
  })
