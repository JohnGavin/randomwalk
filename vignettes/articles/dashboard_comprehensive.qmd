    ...
    column(12,
      tabsetPanel(
        ...
        tabPanel("Fractal Graph",
                 ...
                 tags$p(style = "margin-top: 10px; color: #666; font-style: italic;",
                        "Color shows walker arrival order: Dark blue = early arrivals, Light yellow = late arrivals"),
                 downloadButton("download_fractal", "Download Plot", class = "btn-sm"),
                 hr(),
...
server <- function(input, output, session) {
  ...
  observeEvent(input$run_sim, {
    ...
    tryCatch({
      ...
      # For WebR: Split simulation into smaller chunks with Sys.sleep() to yield control
      if (is_webr()) {
        # Estimate total runtime
        est_steps_per_sec <- 200 / (1 + 0.002 * input$n_walkers)
        est_runtime <- input$max_steps / est_steps_per_sec
        chunks <- 20  # Split into 20 chunks for smooth updates

        # Process simulation in chunks with progress updates
        for (i in 1:chunks) {
          if (i == chunks) {
            # Final chunk: run the actual simulation
            ...
            # Update with real final values
            sim_completed_walkers(result$statistics$completed_walkers)
            sim_black_pixels(result$statistics$black_pixels)
          } else {
            # Intermediate chunks: simulate progress
            progress <- i / chunks
            sim_current_step(floor(input$max_steps * progress))
            sim_completed_walkers(floor(input$n_walkers * progress * 0.8))
            sim_black_pixels(floor(input$n_walkers * progress * 0.7))

            # CRITICAL: Yield control to browser for timer update
            # Sleep for estimated time per chunk
            Sys.sleep(est_runtime / chunks)
          }
        }
      } else {
  ...
  output$paths_plot_first25 <- renderPlot({
    ...
    if (n_walkers >= 1) {
      ...
      if (length(walker_indices) > 0) {
        ...
        # Collect all paths first, then draw them
        all_paths <- list()
        all_starts <- list()
        all_ends <- list()
