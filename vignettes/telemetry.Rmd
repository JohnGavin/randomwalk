...
format_sim_stats <- function(stats) {
  data.frame(
    ...
    Value = c(
      ...
      format(stats$final_black_count, big.mark = ","),
      sprintf("%.2f%%", stats$final_black_percentage),
      sprintf("%d × %d", stats$grid_size, stats$grid_size),
      stats$n_walkers,
      sprintf("%.3f", stats$elapsed_time)
    ),
...
This simulation is configured to achieve substantial grid coverage (target: 25%+):
...
```{r plot-large-grid, fig.cap="Large simulation: Extensive coverage with 12 walkers", fig.width=10, fig.height=10}
tar_read(plot_large_grid)
```

The large simulation demonstrates the package's ability to handle more complex scenarios while maintaining reasonable computation times.
...
```{r pipeline-viz, fig.width=12, fig.height=8, fig.cap="Pipeline dependency graph showing target relationships and execution flow"}
tar_read(pipeline_network)
```
