...
description: "Best practices for validation, error handling, and robust code patterns in the randomwalk package"
...
vignette: >
  %\VignetteIndexEntry{Defensive Programming}
...
- **`stopifnot()`**: Concise, avoids repetitive if statements, named messages show which condition failed
- **`cli` package**: Tidyverse-style formatting with styled text (`{.arg}`, `{.val}`), bullet points for clarity
- **Both approaches**: Reduce code duplication, improve readability, consistent error formatting
...
- [R/simulation.R](https://github.com/JohnGavin/randomwalk/blob/main/R/simulation.R) - Main simulation with validation
- [R/grid.R](https://github.com/JohnGavin/randomwalk/blob/main/R/grid.R) - Grid validation functions
- [Wiki: Issue #157 Diagnostics](https://github.com/JohnGavin/randomwalk/wiki/Issue-157-WebR-Isolated-Pixels)

## Validation Performance Comparison

Compare validation overhead across different grid sizes:

```{r validation-benchmark, eval=TRUE}
#| tbl-cap: "Validation overhead decreases with grid size (% of total time)"
...
```{r validation-errors, eval=TRUE}
#| tbl-cap: "Example validation errors with user-friendly messages"
...
**Learn more**: See [R/simulation.R:70-95](https://github.com/JohnGavin/randomwalk/blob/main/R/simulation.R#L70-L95) for complete parameter validation implementation.
