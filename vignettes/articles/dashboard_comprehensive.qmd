    ...
    mainPanel(
      tabsetPanel(
        ...
        tabPanel("Notes",
                 ...
                 h5("Browser Mode"),
                 tags$p("This dashboard runs in ", strong("sync sequential mode"), " (workers=0)."),
                 tags$p("Each walker is processed one at a time. This is a WebR/browser limitation."),
                 hr(),
                 h5("Features"),
                 tags$ul(
                   tags$li(strong("Multiple visualizations:"), " Fractal graph, walker paths, distributions"),
                   tags$li(strong("Comprehensive statistics:"), " Grid, walker, step, and performance metrics"),
                   tags$li(strong("Debug information:"), " Package versions, environment detection, timing")
                 ),
                 hr(),
                 h5("Related Links"),
                 tags$ul(
                   tags$li(tags$a(href="https://johngavin.github.io/randomwalk/", "randomwalk Package Documentation")),
                   tags$li(tags$a(href="dynamic_broadcasting.html", "Simplified Dashboard"))
                 ),
                 hr(),
                 ...
                 tags$ul(
                   tags$li("Sync sequential only (no parallel processing in browser)"),
                   tags$li("First load can take 15-30 seconds (package downloads)"),
                   tags$li("Large simulations may be slow (consider reducing walkers/grid size)")
                 ))
      )
    )
  )
...
This dashboard provides a **comprehensive interactive Shiny application** for random walk simulations running in your browser using WebAssembly (WebR).

> **Browser Mode:** This dashboard runs in **sync sequential mode** only (workers=0). Each walker is processed one at a time. This is a WebR/browser limitation - async parallel processing requires native R.

## Features

### Visualization Types
- **Fractal Graph**: Black pixel patterns showing the final grid state
...
- **Debug Info**: Package versions, environment detection, timing

### Simulation Controls
- **Grid Size**: 20-400 pixels (larger grids = more exploration space)
...
| Feature | This Dashboard | [Simplified Dashboard](dynamic_broadcasting.html){target="_blank"} |
|---------|---------------|-----------------------------------|
...
- [Simplified Dashboard](dynamic_broadcasting.html)
